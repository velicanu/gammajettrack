{
  ...
  for(int izcut = 1 ; izcut < 2 ; ++izcut)
  {
    ...
    TLegend * legnorm = new TLegend(0.25,0.65,0.55,0.9);
    ...
    for(int itrkcut = itrkstart ; itrkcut < itrkstart+1 ; ++itrkcut)
    {
      stacked_pbpbp_pp[izcut]->SaveAs(Form("gammatrkdphi_pbpb_pp_%d_trkPt_%d.png",(int)trkptmin[itrkcut],(int)trkptmax[itrkcut]));
    }
  }
}
