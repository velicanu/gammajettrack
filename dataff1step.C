...
void dataff1step(int phoetmin, int phoetmax, int jetptmin = 30, int trkptcut = 4, int do_divide = 0, int gammaxi = 0, string inpath="./", string outfname = "outfile.root", int whichpurity = 0) {
  ...
  for (int icent = 0; icent < ncentbins; icent++) {
    ...
    if(do_divide==0) {
      rawffue_pbpbdata_recoreco[icent]->Scale(centscale[icent]);
      rawff_pbpbdata_recoreco[icent]->Scale(centscale[icent]);
      rawffue_pbpbdata_recoreco[icent]->SetMarkerColor(kRed);
      rawffue_pbpbdata_recoreco[icent]->Draw("same");
      rawff_pbpbdata_recoreco[icent]->Draw("same");
    ...
    {
      if(do_divide==0) {
        leg_ff_pbpbsub[icent]->AddEntry(rawff_pbpbdata_recoreco[icent],"Raw PbPb FF","p");
        leg_ff_pbpbsub[icent]->AddEntry(rawffue_pbpbdata_recoreco[icent],"UE PbPb FF","p");
