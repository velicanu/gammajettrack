...
void photonjettrack::jetshape(std::string outfname, int centmin, int centmax, float phoetmin, float phoetmax, int jetptcut, std::string gen, int checkjetid, int trkptmin, int gammaxi) {
  ...
  // TH1D * hgammaffxiuesiggen = new TH1D(Form("hgammaffxiuesiggen_%s_%s_%d_%d",tag.data(),s_alpha.data(),abs(centmin),abs(centmax)),Form(";r;#rho(r)"),20,0,1); // Raw FF after cuts (1.0)
  // TH1D * hgammaffxiuemixgen = new TH1D(Form("hgammaffxiuemixgen_%s_%s_%d_%d",tag.data(),s_alpha.data(),abs(centmin),abs(centmax)),Form(";r;#rho(r)"),20,0,1); // Raw FF after cuts (1.0)
    ...
    for (ij = 0; ij < nij; ij++) {
      ...
      for (ip = 0; ip < nip; ++ip) {
        ...
        if (dr < 1) {
          ...
          if (signal) { hgammaffxi->Fill(deltar, p_pt[ip] / refpt * weight * getTrkWeight(ip, trkWeight, gen)); }
          if (sideband) { hgammaffxisideband->Fill(deltar, p_pt[ip] / refpt * weight * getTrkWeight(ip, trkWeight, gen)); }
        }
      }
      ...
      for (ip_mix = 0; ip_mix < nip_mix; ++ip_mix) {
        ...
        if (dr < 1) {
          ...
          if (signal) { hgammaffxiuemix->Fill(deltar, p_pt_mix[ip_mix] / refpt * weight * getTrkWeight(ip_mix, trkWeight_mix, gen) / nmixedUEevents); }
          if (sideband) { hgammaffxiuemixsideband->Fill(deltar, p_pt_mix[ip_mix] / refpt * weight * getTrkWeight(ip_mix, trkWeight_mix, gen) / nmixedUEevents); }
        }
      }
    }
    ...
    for (ij_mix = 0; ij_mix < nij_mix; ij_mix++) {
      ...
      for (int ip_mix = 0; ip_mix < nip_mix; ++ip_mix) {
        ...
        if (dr < 1) {
          ...
          if (sideband) {
            hgammaffxijetmixsideband->Fill(deltar, p_pt_mix[ip_mix] / refpt * weight * getTrkWeight(ip_mix, trkWeight_mix, gen) / nmixedjetevents);
          }
        }
      }
      ...
      for (int ip_mix = 0; ip_mix < nip_mix; ++ip_mix) {
        ...
        if (dr < 1) {
          ...
          if (sideband) {
            hgammaffxijetmixuesideband->Fill(deltar, p_pt_mix[ip_mix] / refpt * weight * getTrkWeight(ip_mix, trkWeight_mix, gen) / nmixedjetevents);
          }
        }
      }
    }
