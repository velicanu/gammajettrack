...
void dijetSkim(TString infilename="HiForest.root", TString outfilename="Zevents.root", string jetname="ak4PFJetAnalyzer") {
...
int main(int argc, char *argv[])
{
  if((argc < 3))
  {
    std::cout << "Usage: ./dijetSkim.exe <inputfile> <outputfile> [jetname]" << std::endl;
    return 1;
  }
  if(argc==3)  dijetSkim(argv[1], argv[2]);
  if(argc==4)  dijetSkim(argv[1], argv[2], argv[3]);
  return 0;
}
