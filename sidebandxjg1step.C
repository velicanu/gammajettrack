...
void sidebandxjg1step(int phoetmin, int phoetmax, int jetptmin)
{
  ...
  for (int icent = 0; icent < nCentBins; icent++) {
    ...
    h1D_xjg_ptBin1_hiBin_phoSIG_jetSIG_final_norm[icent] = (TH1D*) _fran->Get(Form("HI/h1D_xjg_ptBin1_hiBin%d_phoBKG_jetRAW_final_norm",icent+3));

    xjgsideband_pbpbdata_recoreco[icent]->Sumw2();
    xjgmixsideband_pbpbdata_recoreco[icent]->Sumw2();

    float nphosideband = phoetsideband_pbpbdata_recoreco[icent]->GetEntries();
    float binwidth = xjgsideband_pbpbdata_recoreco[icent]->GetBinWidth(1);
    ...
    xjgsideband_pbpbdata_recoreco[icent]->Divide(h1D_xjg_ptBin1_hiBin_phoSIG_jetSIG_final_norm[icent]);
    xjgsideband_pbpbdata_recoreco[icent]->Draw("same");
    ...
    if(icent==0)
    {
      leg[icent]->AddEntry(xjgsideband_pbpbdata_recoreco[icent],"PbPb #sqrt{s_{NN}}=5 TeV","p");
      leg[icent]->AddEntry(h1D_xjg_ptBin1_hiBin_phoSIG_jetSIG_final_norm[icent],"HIN-16-002","p");
    }
    else if(icent==1)
    {
      leg[icent]->AddEntry(xjgsideband_pbpbdata_recoreco[icent],Form("%d>#gamma p_{T}>%d GeV",phoetmin,phoetmax),"");
      leg[icent]->AddEntry(xjgsideband_pbpbdata_recoreco[icent],"0.11<#sigma_{I#eta#eta}<0.17","");
    }
    else if(icent==2)
    {
      leg[icent]->AddEntry(xjgsideband_pbpbdata_recoreco[icent],"|#eta^{jet}| < 1.6","");
      leg[icent]->AddEntry(xjgsideband_pbpbdata_recoreco[icent],Form("p_{T}^{jet} > %d GeV",jetptmin),"");
    }
    else if(icent==3)
    {
      leg[icent]->AddEntry(xjgsideband_pbpbdata_recoreco[icent],"","");
      leg[icent]->AddEntry(xjgsideband_pbpbdata_recoreco[icent],"","");
    }
    leg[icent]->AddEntry(xjgsideband_pbpbdata_recoreco[icent],Form("%s",cents[icent].data()),"");
  ...
  TLatex * ldndxi = new TLatex(0.4,0.27,"1/N#gamma dN/dx_{J#gamma} Raw Sideband");
  ...
  c1->SaveAs("xjg_step1_allsubtractions_sideband.png");
}
