...
//! (2.1) Event selections
    ...
    if( (phoEtCorrected[0]*phoerror)<phoetmin || (phoEtCorrected[0]*phoerror)>phoetmax) continue;
    if(signal) {
      phoetsignal->Fill((phoEtCorrected[0]*phoerror));
    }
    if(sideband) {
      phoetsideband->Fill((phoEtCorrected[0]*phoerror));
    }
...
//! jet selections
      ...
      if(signal) {
        ...
        njets_perevent++;
        if(jetptCorr[ijet]!=-1) { xjgsignal->Fill(jetptCorr[ijet]/(phoEtCorrected[0]*phoerror)); }
        else                    { xjgsignal->Fill(jetpt[ijet]/(phoEtCorrected[0]*phoerror)); }
      }
      if(sideband) {
        hjetptsideband->Fill(tmpjetpt);
        if(jetptCorr[ijet]!=-1) { xjgsideband->Fill(jetptCorr[ijet]/(phoEtCorrected[0]*phoerror)); }
        else                    { xjgsideband->Fill(jetpt[ijet]/(phoEtCorrected[0]*phoerror)); }
      }
      ...
      if(gen.compare("recogen")==0)
      {
        for(int igen = 0 ; igen < mult ; ++igen)
        {
          ...
          if(dr<0.3)
          {
            ...
            if(gammaxi==1) z = pt[igen]*cos(angle)/(phoEtCorrected[0]*phoerror);
            float xi = log(1.0/z);
        ...
        for(int igen_mix = 0 ; igen_mix < mult_mix ; ++igen_mix)
        {
          ...
          if(dr<0.3)
          {
            ...
            if(gammaxi==1) z = pt_mix[igen_mix]*cos(angle)/(phoEtCorrected[0]*phoerror);
            float xi = log(1.0/z);
      ...
      else if(gen.compare("recoreco")==0)
      {
        for (int itrk = 0; itrk < nTrk; itrk++) { // reco jet reco track
          ...
          if(dr<0.3)
          {
            ...
            if(gammaxi==1) z = trkPt[itrk]*cos(angle)/(phoEtCorrected[0]*phoerror);
            float xi = log(1.0/z);
//! 1-1: rjet rtrk
            if(signal) {
              something = true;
      ...
      if(gen.compare("recoreco")==0) {
        ...
        for(int itrk_mix = 0 ; itrk_mix < nTrk_mix ; ++itrk_mix) { // reco jet UE track
          ...
          if(dr<0.3)
          {
            ...
            if(gammaxi==1) z = trkPt_mix[itrk_mix]*cos(angle)/(phoEtCorrected[0]*phoerror);
            float xi = log(1.0/z);
//! 1-3: rjet rtrk_mix
            if(signal) {
      ...
      if(gen.compare("gengen")==0 || gen.compare("mixgen")==0)
      {
        for(int igentrk = 0 ; igentrk < mult ; ++igentrk)
        {
          ...
          if(dr<0.3)
          {
            ...
            if(gammaxi==1) z = pt[igentrk]*cos(angle)/(phoEtCorrected[0]*phoerror);
            float xi = log(1.0/z);
      ...
      if(gen.compare("mixgen")==0)
      {
        for(int igentrk_mix = 0 ; igentrk_mix < mult_mix ; ++igentrk_mix)
        {
          ...
          if(dr<0.3)
          {
            ...
            if(gammaxi==1) z = pt_mix[igentrk_mix]*cos(angle)/(phoEtCorrected[0]*phoerror);
            float xi = log(1.0/z);
...
//! (2.4) Mix reco jet loop
    ...
    for (int ijet_mix = 0; ijet_mix < njet_mix; ijet_mix++) {
      ...
      if(signal && nmixEv_mix[ijet_mix]%3==1) {
        ...
        njets_permixevent++;
        hnmixsignal->Fill(1);
        xjgmixsignal->Fill(jetpt_mix[ijet_mix]/(phoEtCorrected[0]*phoerror),1/nmixedjetevents);
      }
      if(sideband && nmixEv_mix[ijet_mix]%3==1) {
        hjetptjetmixsideband->Fill(jetpt_mix[ijet_mix],1./nmixedjetevents);
        hnmixsideband->Fill(1);
        xjgmixsideband->Fill(jetpt_mix[ijet_mix]/(phoEtCorrected[0]*phoerror),1/nmixedjetevents);
      }
      TLorentzVector vjet;
...
//! 1-2: rjet_mix rtrk_mix
            if(signal) {
              hgammaffxijetmix->Fill(xi,trkWeight_mix[itrk_mix]/nmixedjetevents);
            }
            if(sideband) {
              hgammaffxijetmixsideband->Fill(xi,trkWeight_mix[itrk_mix]/nmixedjetevents);
            }
          }
        }
...
//! 1-4: rjet_mix rtrk_mix
            if(signal) {
              hgammaffxijetmixue->Fill(xi,trkWeight_mix[itrk_mix]/nmixedjetevents);
            }
            if(sideband) {
              hgammaffxijetmixuesideband->Fill(xi,trkWeight_mix[itrk_mix]/nmixedjetevents);
            }
          }
        }
      }
