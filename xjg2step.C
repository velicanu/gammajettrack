...
void xjg2step(int phoetmin, int phoetmax, int jetptmin)
{
  ...
  for (size_t icent = 0; icent < nCentBins; icent++) {
    ...
    xjgsideband_pbpbdata_reco[icent]->Draw("same");
    ...
    if(icent==0)
    {
      leg[icent]->AddEntry(xjgsignal_pbpbdata_reco[icent],"Raw-Mix x_{J#gamma} #sigma_{i#eta#eta}<0.10","p");
  ...
  c1->SaveAs("xjg_step2_signalsideband.png");
}
