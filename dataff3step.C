...
void dataff3step(int phoetmin, int phoetmax, int jetptmin = 30, int trkptcut = 4, int do_divide = 0, int gammaxi = 0, string inpath="./", string outfname = "outfile.root", int whichpurity = 0) {
  ...
  for (int icent = 0; icent < ncentbins; icent++) {
    ...
    if(do_divide==0) {
      rawffsideband_pbpbdata_recoreco[icent]->SetMarkerColor(kRed);
      rawffsideband_pbpbdata_recoreco[icent]->Draw("same");
      rawff_pbpbdata_recoreco[icent]->Draw("same");
    ...
    {
      if(do_divide==0) {
        leg_ff_pbpbsub[icent]->AddEntry(rawff_pbpbdata_recoreco[icent],"Signal #gamma PbPb FF","p");
        leg_ff_pbpbsub[icent]->AddEntry(rawffsideband_pbpbdata_recoreco[icent],"Sideband #gamma PbPb FF","p");
      } else {
