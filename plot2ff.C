...
int plot2ff(const char* file1, const char* file2, const char* plot_name, int draw_log_scale) {
    ...
    for (int i=0; i<4; ++i) {
        ...
        if (i == 0) {
            ...
            for (std::size_t m=0; m<hist_names.size()/5; ++m)
                l1->AddEntry(h1[0][m], hist_names[5*m].c_str(), "p");
...
int main(int argc, char* argv[]) {
    if (argc == 5)
        return plot2ff(argv[1], argv[2], argv[3], atoi(argv[4]));
    else
        printf("./plot2ff <results file> <sys file> <plot name> <log scale>\n");
