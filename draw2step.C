...
void draw2step(int phoetmin, int phoetmax, int jetptmin = 30) {
  ...
  for (int icent = 0; icent < 5; icent++) {
    ...
    hgammaffxiuemix_pbpbmc_reco[icent]->Scale(1/nrawjets);
    ...
    if(icent==0)
    {
      leg_ff_pbpbsub[icent]->AddEntry(rawff_pbpbmc_reco[icent],"Raw FF - Jet Mix FF","p");
      leg_ff_pbpbsub[icent]->AddEntry(hgammaffxiuemix_pbpbmc_reco[icent],"UE Mix FF","p");
    }
    else if(icent==1)
    {
